...
var mdlines = (() => {
  ...
  // Would this write empty a file that had content? Almost always a bug
  // somewhere upstream rather than something the user asked for, so it is
  // refused unless it is forced.
  var emptied = (next, previous) =>
    normalise(next || '') === '' && normalise(previous || '') !== ''
